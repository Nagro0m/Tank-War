...
TankCreationLevel::TankCreationLevel(const int _tankNumber) : Level("TankCreationLevel")
{
...
vector<Track> TankCreationLevel::GetTracks()
{
...
vector<Hull> TankCreationLevel::GetHulls()
{
    vector<Hull> _hulls;
...
vector<Weapon> TankCreationLevel::GetWeapons()
{
...
void TankCreationLevel::GenerateTankCreationMenu()
{
    ...
    FollowCamera* _camera = SpawnCamera<FollowCamera>(FollowCamera(Vector2f(0.0f, 0.0f), Vector2f(1920.0f, 1080.0f), "TankCreationMenu_" + to_string(tankNumber)));
    ...
    //TODO FIW WITH SpawnWidget()
   /* menuLabel = SpawnWidget<UI::Label>(UI::Label("Chenille", Screen, "Gagalin", OTF));
    ...
    // TODO SetCurrentCamera();
}

void TankCreationLevel::PreviousOption()
{
...
void TankCreationLevel::NextOption()
{
    RemoveCurrentSelection();
...
void TankCreationLevel::SpawnCurrentSelection()
{
    if (currentStep == 0)
    {
...
void TankCreationLevel::RemoveCurrentSelection()
{
    if (currentActor)
    {
...
void TankCreationLevel::ConfirmSelection()
{
    if (currentStep < 2)
    {
        currentStep++;
...
void TankCreationLevel::FinalizeTank()
{
    //vector<Code> _tank1 =
    //{
    //	Code::Q,
